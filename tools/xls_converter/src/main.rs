...
use mm_file_formats::adf::{AdfFile, TYPE_LIBRARIES};
...
fn main() -> anyhow::Result<()> {
    ...
    match extension {
        "xlsc" => {
            ...
            let instance = adf
                .get_instance_by_info::<XlsBook>("XLSBook")
                .context("failed to find matching instance")?;

            // Read associated instance
            let book: XlsBook = instance.read()?;
                ...
                for y in 0..sheet.rows {
                    ...
                    for x in 0..sheet.cols {
                        ...
                        let foreground_color = book
                            .color_data
                            .get((attributes.fg_color_index % book.color_data.len() as u8) as usize)
                            .cloned()
                            .context("Failed to get foreground color")?;
                        let background_color = book
                            .color_data
                            .get((attributes.bg_color_index % book.color_data.len() as u8) as usize)
                            .cloned()
            ...
            let mut serializer = quick_xml::se::Serializer::with_root(&mut buffer, Some("book"))?;
            serializer.indent('\t', 1);
        ...
        "xml" => {
            ...
            adf.instances.clear();
            adf.description = "".into();

            // Write book to new instance
            let instance = adf
                .new_instance_from_info::<XlsBook>("XLSBook")
                .context("Failed to create instance")?;
            instance.write(&XlsBook {
                sheet: sheets.into(),
