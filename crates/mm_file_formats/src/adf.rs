...
impl BinRead for AdfFile {
    ...
    fn read_options<R: std::io::Read + std::io::Seek>(
        ...
        // This is going to be very painful. Creating a buffer from scratch will require a scratch buffer that can grow/shrink.
        // Though on the one hand, that means these buffers are trivially relocatable, and we just need a simple free list / grow when necessary
        // Leaving all existing relative pointers untouched when growing, and doing a memcpy into a larger buffer
        // Maybe not as difficult as imagined, and we can do a separate pass to compact data on write if we really need to
